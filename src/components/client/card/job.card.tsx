...
const JobCard = (props: any) => {
  ...
  useEffect(() => {
    fetchJob().then((res) => {
      if (props.keyword && res) {
        setDisplayJob(res.filter((v) => v.name?.includes(props.keyword)));
      }
    });
  }, [props.keyword]);
  ...
  useEffect(() => {
    fetchJob().then((res) => {
      if (props.skill && res) {
        setDisplayJob(res.filter((v) => v.skills?.includes(props.skill)));
      }
    });
  }, [props.skill]);
  useEffect(() => {
    fetchJob().then((res) => {
      if (props.address && res) {
        setDisplayJob(res.filter((v) => v.location?.includes(props.address)));
      }
    });
  }, [props.address]);
